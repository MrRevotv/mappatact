    ...
    map.on('zoomend', () => {
        ...
        Object.keys(markerSquadre).forEach(id => {
            ...
            if (ic.options.iconSize) {
                ic.options.iconSize = [80 * factor, 80 * factor];
                ic.options.iconAnchor = [40 * factor, 40 * factor];
                mk.setIcon(ic);
            }
        });
    });
...
window.toggleOrbita = () => {
    const o = document.getElementById('sidebar-orbita');
    const b = document.getElementById('btn-toggle-orbita');
    o.classList.toggle('nascosto');
    b.innerText = o.classList.contains('nascosto') ? "👁️ Mostra Orbita" : "👁️ Nascondi Orbita";
}
...
function creaMarker(dati) {
    ...
    const icon = L.divIcon({ html: html, className: 'wrapper-icona', iconSize: [80 * currentFactor, 80 * currentFactor], iconAnchor: [40 * currentFactor, 40 * currentFactor] });
...
window.toggleTerminal = (e) => {
    ...
    document.getElementById('terminal-status').innerText = t.classList.contains('minimizzato') ? "(Clicca per espandere)" : "(Clicca per nascondere)";
}
...
window.aggiornaIconaVisiva = (id, nuovoPercorso) => {
    ...
    m.setIcon(L.divIcon({ html: html, className: 'wrapper-icona', iconSize: [80 * currentFactor, 80 * currentFactor], iconAnchor: [40 * currentFactor, 40 * currentFactor] }));
};
...
// ==========================================
// FUNZIONE PER TRASCINARE IL SET OPERATIVO
// ==========================================
function attivaDragPannello() {
    const pannello = document.getElementById('pannello');
    const handle = document.querySelector('.tab-pannello');
    if (!pannello || !handle) return;
    ...
    handle.addEventListener('mousedown', (e) => {
        ...
        // Calcola la posizione attuale in pixel per sganciarlo dal fondo (bottom)
        ...
        pannello.style.left = initialLeft + 'px';
        pannello.style.top = initialTop + 'px';
    ...
    function onMouseMove(e) {
        ...
        // Se si muove di oltre 3 pixel, consideralo un "drag" e non un semplice "click"
        if (Math.abs(dx) > 3 || Math.abs(dy) > 3) pannello.classList.add('in-drag');

        pannello.style.left = (initialLeft + dx) + 'px';
        pannello.style.top = (initialTop + dy) + 'px';
    }
...
// Riscriviamo il toggle per ignorare il click se abbiamo appena trascinato
window.togglePannello = () => {
    const pannello = document.getElementById('pannello');
    if (pannello.classList.contains('in-drag')) {
        pannello.classList.remove('in-drag');
        return; // Annulla la chiusura se stavamo trascinando
    }
    pannello.classList.toggle('nascosto');
};

// Attiva il drag un secondo dopo l'avvio della pagina
setTimeout(attivaDragPannello, 1000);
